...
"""Azure OpenAI agent for document Q&A with ColQwen retrieval."""
...
class ColQwenAgent:
    def __init__(self, deployment_name: Optional[str] = None):
        ...
        if not self.open_ai_endpoint:
            raise ValueError(
                "Azure AI project endpoint is not configured. Set AZURE_AI_open_ai_endpoint or AI_FOUNDRY_OPEN_AI_ENDPOINT."
            )
        ...
        logging.info(
            "Enhanced ColQwenAgent initialized for model deployment '%s' using Azure OpenAI Chat Client.",
            self.deployment_name,
        )
